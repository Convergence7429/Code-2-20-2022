...
public class Shooter {
    ...
    final static float y = 104.016f; // in // upper hub height by default. Should have button to switch between them
    ...
    final static float horizontalDistanceFromLimeLightToShooter = 5.5f; // in
    final static float horizontalDistanceFromShooterToFrontOfRobot = 7.0f;
    final static float horizontalDistanceFromLimelightToFrontOfRobot = horizontalDistanceFromShooterToFrontOfRobot - horizontalDistanceFromLimeLightToShooter;
    ...
    private static float angleError = 0.00f; // was 0.04

    static double calculatedVelocity = 0.0f;
    static double calculatedAngle = 0.000f;


    public static void testVelocity(float x) {

        float velocity = 0.00f;
        for (float i = minimumVelocity; i <= maximumVelocity; i += 0.1) {
    ...
    public static float testAngle(float velocity, float x) {
        ...
        float[] possibleAngles = new float[(int) ((maximumAngle * 1000.0f) - (minimumAngle * 1000.0f) + 1.0f)];
    ...
    public static float slopeOfLine(float velocity, float possibleAngle, float x) {
        ...
        return (float) ((y - x) / (((-1) * ((-1) * velocity * Math.sin(possibleAngle) - Math.sqrt(Math.pow(velocity, 2)
                * Math.pow(Math.sin(possibleAngle), 2) + ((2 * gravity) * (y0 - y)))) / gravity)
                - (x / (velocity * Math.cos(possibleAngle)))));
    }

    public static float getXDistanceFromCenterOfHub(float verticalAngle) { // x distance from shooter to center of hub
        return (float) ((y - cameraHeight) / Math.tan(Math.toRadians(verticalAngle + cameraAngle - angleError))) + distanceFromTapeToCenterOfHub + horizontalDistanceFromLimeLightToShooter; // ADD DISTANCE FROM TAPE TO CENTER
    }

    /*public static float getXDistanceFromCenterOfHub(float verticalAngle) { // x distance from front of robot to fender. better for testing
        return (float) ((y - cameraHeight) / Math.tan(Math.toRadians(verticalAngle + cameraAngle - angleError))) - horizontalDistanceFromLimelightToFrontOfRobot - distanceFromFenderToTape; // ADD DISTANCE FROM TAPE TO CENTER
    }*/

    public static float shooterWheelLinearVelocityToMotorVelocity(double shooterWheelLinearVelocity){ // method with coefficient
        return (float) (2.0f * (shooterWheelLinearVelocity * (1.00f/(4.00f*Math.PI)) * (1.00f/1.21f) * 2048.0f * (1.0f/10.0f)));
    ...
    public void testingShooter(){
        ...
        if(NetworkTableInstance.getDefault().getTable("limelight").getEntry("tv").getBoolean(false)){
            System.out.println(getXDistanceFromCenterOfHub((float) NetworkTableInstance.getDefault().getTable("limelight").getEntry("ty").getDouble(0.00)));
        } else {
        ...
        if(Constants.stick.getRawButtonPressed(1)){
            ...
            if(shooting){
                ...
                shooterTimer.reset();
                shooterTimer.start();
                testVelocity(getXDistanceFromCenterOfHub((float) NetworkTableInstance.getDefault().getTable("limelight").getEntry("ty").getDouble(0.0)));
